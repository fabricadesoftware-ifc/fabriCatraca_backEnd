...
- No sistema da Catraca existe atualmente uma relação NxM entre `area` e `access_rules`, provavelmente já planejada para gerenciar diferentes áreas com regras de acesso diferentes, porem atualmente no sistema essa realção não existe, ou seja, os meios de liberação atualmente estão atrelados a grupos ou regras ao usuario. Pode ser facilemte corrijido criando a nova model AreaAccessRule, e criar seu mixin.
