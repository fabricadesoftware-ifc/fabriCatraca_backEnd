...
def config_root(request, format=None):
    return Response(
        {
            "system_configs": reverse(
                "systemconfig-list", request=request, format=format
            ),
            "hardware_configs": reverse(
                "hardwareconfig-list", request=request, format=format
            ),
            "security_configs": reverse(
                "securityconfig-list", request=request, format=format
            ),
            "ui_configs": reverse("uiconfig-list", request=request, format=format),
            "catra_configs": reverse(
                "catraconfig-list", request=request, format=format
            ),
            "push_server_configs": reverse(
                "pushserverconfig-list", request=request, format=format
            ),
            "easy_setup": reverse("easy-setup", request=request, format=format),
            "easy_setup_history": reverse(
                "easy-setup-history", request=request, format=format
            ),
            "sync_all_configs": reverse(
                "sync-all-configs", request=request, format=format
            ),
            "sync_config_status": reverse(
                "sync-config-status", request=request, format=format
            ),
...
urlpatterns = [
    ...
    path(
        "easy-setup/status/<str:task_id>/", easy_setup_status, name="easy-setup-status"
    ),
    path("easy-setup/history/", easy_setup_history, name="easy-setup-history"),
    ...
    path(
        "device-config/<int:device_id>/", sync_device_config, name="sync-device-config"
    ),
