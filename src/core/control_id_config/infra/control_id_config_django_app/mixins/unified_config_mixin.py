...
class UnifiedConfigSyncMixin(ControlIDSyncMixin):
                ...
                config, created = SystemConfig.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "clear_expired_users": to_bool(
                            config_data.get("clear_expired_users"), False
                        ),
                        "url_reboot_enabled": to_bool(
                            config_data.get("url_reboot_enabled"), True
                        ),
                        "keep_user_image": to_bool(
                            config_data.get("keep_user_image"), True
                        ),
                        "catra_timeout": config_data.get("catra_timeout", 30),
                        "online": to_bool(config_data.get("online"), True),
                        "local_identification": to_bool(
                            config_data.get("local_identification"), True
                        ),
                        "language": config_data.get("language", "pt"),
                        "daylight_savings_time_start": config_data.get(
                            "daylight_savings_time_start"
                        ),
                        "daylight_savings_time_end": config_data.get(
                            "daylight_savings_time_end"
                        ),
                        "web_server_enabled": to_bool(
                            config_data.get("web_server_enabled"), True
                        ),
                    },
                )
            ...
            if config_data:
                config, created = HardwareConfig.objects.update_or_create(
                    ...
                    defaults={
                        ...
                        "relayN_enabled": to_bool(
                            config_data.get("relayN_enabled"), False
                        ),
                        "relayN_timeout": config_data.get("relayN_timeout", 5),
                        "relayN_auto_close": to_bool(
                            config_data.get("relayN_auto_close"), True
                        ),
                        "door_sensorN_enabled": to_bool(
                            config_data.get("door_sensorN_enabled"), False
                        ),
                        "door_sensorN_idle": config_data.get("door_sensorN_idle", 10),
                        "doorN_interlock": to_bool(
        ...
        try:
            ...
            if config_data:
                config, created = UIConfig.objects.update_or_create(
                    device=self.device,
                    defaults={
                        "screen_always_on": to_bool(
                            config_data.get("screen_always_on"), False
                        )
                    },
                )
    ...
    def sync_monitor_config_from_catraca(self):
        ...
        try:
            ...
            # Apenas campos básicos suportados por toda catraca Control iD
            base_fields = ["request_timeout", "hostname", "port", "path"]
            payloads = [
                # 1) Bloco direto (retorna todos os campos do monitor)
                {"monitor": {}},
                # 2) Bloco direto com fields explícitos (só básicos)
                {"monitor": {"fields": base_fields}},
                # 3) Via config.modules (name/fields)
                {"config": {"modules": [{"name": "monitor", "fields": base_fields}]}},
            ...
            for body in payloads:
                ...
                resp = self._make_request(
                    "get_configuration.fcgi", method="POST", json_data=body
                )
                ...
                if isinstance(raw.get("config"), dict):
                    modules = raw["config"].get("modules") or raw["config"].get(
                        "module"
                    )
                                ...
                                if isinstance(params, list):
                                    for p in params:
                                        ...
                                        if k is not None:
                                            parsed[k] = v
                                fields_block = (
                                    mod.get("fields") or mod.get("values") or {}
                                )
                ...
                if isinstance(raw.get("config"), dict):
                    ...
                    if isinstance(mods, list):
                        last_meta["module_names"] = [
                            m.get("name") or m.get("module") for m in mods
                        ]
    ...
    def update_monitor_config_in_catraca(self, values: dict) -> Response:
        ...
        try:
            ...
            probe = self._make_request(
                "get_configuration.fcgi", method="POST", json_data={"monitor": {}}
            )
            ...
            filtered = {
                k: ("" if v is None else str(v)) for k, v in (values or {}).items()
            }
            ...
            response = self._make_request(
                "set_configuration.fcgi", method="POST", json_data={"monitor": filtered}
            )
            if response.status_code != 200:
                return Response(
                    {
                        "success": False,
                        "error": response.text
                        or "Erro ao salvar configurações de monitor",
                    },
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
