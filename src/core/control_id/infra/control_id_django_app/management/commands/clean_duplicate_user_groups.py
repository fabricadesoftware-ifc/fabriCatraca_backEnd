    ...
    def handle(self, *args, **options):
        ...
        if verbose:
            ...
            for (user_id, group_id), relations in duplicates.items():
                ...
                self.stdout.write(
                    f"  Usuario: {user.name} (ID: {user_id}) -> Grupo: {group.name} (ID: {group_id})"
                )
                self.stdout.write(f"     {len(relations)} instancias encontradas")
                for i, relation in enumerate(relations):
                    self.stdout.write(
                        f"       - ID: {relation.id} {'(MANTER)' if i == 0 else '(REMOVER)'}"
                    )
                self.stdout.write("")
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    f"DRY RUN: {duplicate_count} relacoes seriam removidas"
                )
            )
        ...
        with transaction.atomic():
            for (user_id, group_id), relations in duplicates.items():
                ...
                if verbose:
                    ...
                    group = to_keep.group
                    self.stdout.write(
                        f"Removendo {len(to_remove)} duplicatas de {user.name} -> {group.name}"
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Limpeza concluida! {removed_count} relacoes duplicadas removidas."
            )
        )
