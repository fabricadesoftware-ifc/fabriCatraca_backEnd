...
class DeviceViewSet(viewsets.ModelViewSet):
    ...
    def _inspect_remote_registry(self, device: Device):
        ...
        remote_registry = mixin.load_objects(
            "devices", fields=["id", "name", "ip"], order_by=["id"]
        )
        remote_by_id = {
            int(item["id"]): item
            for item in remote_registry
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.validated_data.get("is_default"):
            Device.objects.filter(is_default=True).exclude(id=instance.id).update(
                is_default=False
            )
    ...
    def logo_image(self, request, pk=None, slot_id=None):
        ...
        return Response(
            {
                "success": False,
                "error": f"Nenhum logo encontrado no slot {slot_id_int}",
            },
    ...
    def upload_logo(self, request, pk=None, slot_id=None):
        ...
        if not file_obj:
            return Response(
                {"success": False, "error": "Arquivo PNG é obrigatório"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if file_obj.size > 1024 * 1024:
            return Response(
                {"success": False, "error": "Logo não pode ultrapassar 1MB"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def delete_logo(self, request, pk=None, slot_id=None):
        ...
        if active_slot == slot_id_int:
            mixin._make_request(
                "set_configuration.fcgi", json_data={"general": {"show_logo": "0"}}
            )
    ...
    def show_logo(self, request, pk=None):
        ...
        if slot_id < 0 or slot_id > 8:
            return Response(
                {"success": False, "error": "slot_id deve estar entre 0 e 8"},
                status=status.HTTP_400_BAD_REQUEST,
            )
