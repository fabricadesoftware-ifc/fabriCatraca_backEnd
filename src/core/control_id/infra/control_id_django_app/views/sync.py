...
class GlobalSyncMixin(ControlIDSyncMixin):
    ...
    def sync_time_zones(self, device):
        ...
        time_zones = self.load_objects(
            "time_zones", fields=["id", "name"], order_by=["id"]
        )
        return time_zones
    ...
    def sync_access_rules(self, device):
        ...
        access_rules = self.load_objects(
            "access_rules", fields=["id", "name", "type", "priority"], order_by=["id"]
        )
        return access_rules
    ...
    def sync_areas(self, device):
        """Sincroniza áreas"""
        areas = self.load_objects("areas", fields=["id", "name"], order_by=["id"])
        return areas
    ...
    def sync_cards(self, device):
        ...
        cards = self.load_objects(
            "cards", fields=["user_id", "value"], order_by=["user_id"]
        )
        return cards
    ...
    def sync_groups(self, device):
        """Sincroniza grupos"""
        groups = self.load_objects("groups", fields=["id", "name"], order_by=["id"])
        return groups
...
def sync_device_config(request, device_id: int):
    ...
    try:
        device = (
            User.objects.none()
        )  # placeholder to avoid unused import; real device fetched below
            ...
            return Response(
                {
                    ...
                    "hardware": HardwareConfigSerializer(hw_cfg).data
                    if hw_cfg
                    else None,
                    "security": SecurityConfigSerializer(sec_cfg).data
                    if sec_cfg
                    else None,
...
def sync_status(request):
    ...
    if not task_id:
        return Response(
            {"error": "task_id é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
        )
