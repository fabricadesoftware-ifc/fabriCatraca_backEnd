            ...
            'properties': {
                'object_changes': {
                    ...
                    'items': {
                        'type': 'object',
                        'properties': {
                            'object': {'type': 'string', 'enum': ['access_logs', 'templates', 'cards', 'alarm_logs']},
                            'type': {'type': 'string', 'enum': ['inserted', 'updated', 'deleted']},
                            'values': {'type': 'object'}
                        }
                    }
                },
...
def receive_dao_notification(request):
    ...
    try:
        ...
        logger.debug(f"📥 [MONITOR] Payload: {request.data}")
        ...
        if not isinstance(object_changes, list):
            ...
            return Response({
                'success': False,
                'error': 'object_changes deve ser uma lista'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        if result.get('success'):
            logger.info(f"✅ [MONITOR] Notificação processada com sucesso: {result.get('processed')} itens")
            return Response(result, status=status.HTTP_200_OK)
        else:
            logger.error(f"❌ [MONITOR] Erro processando notificação: {result.get('error')}")
