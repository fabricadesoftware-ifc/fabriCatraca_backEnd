...
class AccessRuleSerializer(serializers.ModelSerializer):
    ...
    def get_time_zones(self, obj):
        links = AccessRuleTimeZone.objects.filter(access_rule=obj).select_related(
            "time_zone"
        )
        return [
            {
    ...
    def get_areas(self, obj):
        links = PortalAccessRule.objects.filter(access_rule=obj).select_related(
            "portal__area_from", "portal__area_to"
        )
        areas_by_id = {}
        for link in links:
            portal = link.portal
