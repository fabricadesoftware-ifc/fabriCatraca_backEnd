...
class DeviceRegistrySyncService:
    ...
    def sync_target_device(self, target_device, desired_values=None):
        ...
        current_rows = self.sync.load_objects(self.OBJECT_NAME, fields=self.FIELDS, order_by=["id"])
        current_by_id = self._normalize_rows(current_rows)

        create_values = [item for item_id, item in desired_by_id.items() if item_id not in current_by_id]
        update_values = [
            item
            for item_id, item in desired_by_id.items()
            if item_id in current_by_id and current_by_id[item_id] != item
        ]
        delete_ids = [item_id for item_id in current_by_id if item_id not in desired_by_id]

        if create_values:
            self._post("create_objects.fcgi", {"object": self.OBJECT_NAME, "values": create_values})

        if update_values:
            self._post("modify_objects.fcgi", {"object": self.OBJECT_NAME, "values": update_values})

        for delete_id in delete_ids:
            self._post(
                "destroy_objects.fcgi",
                {"object": self.OBJECT_NAME, "where": {self.OBJECT_NAME: {"id": delete_id}}},
            )
    ...
    def sync_all_active_devices(self, target_devices: Iterable[Device] | None = None):
        ...
        targets = list(target_devices) if target_devices is not None else list(Device.objects.filter(is_active=True).order_by("id"))
        results = []
        for target_device in targets:
            try:
                results.append(self.sync_target_device(target_device, desired_values=desired_values))
            except Exception as exc:
                logger.exception("Falha ao sincronizar registry de devices para %s", target_device.name)
                results.append(
                    {
                        "device_id": target_device.id,
                        "device_name": target_device.name,
